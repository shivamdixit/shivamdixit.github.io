...
Though the command didn't execute, _"command not found"_ error was printed on the stderr and stdout was blank. The symlink sign `->` has a hidden redirection operator `>`, which caused the output of stdout (which is empty) to overwrite the contents of index.php file.
